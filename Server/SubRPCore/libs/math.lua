...
function LuaVec3:__tostringDeg()
  return string.format('vec3(%.9g,%.9g,%.9g)', math.deg(self.x), math.deg(self.y), math.deg(self.z))
end
...
function LuaVec3:toPoint3F()
  return self -- backwards compatibility
end
...
function LuaVec3:getAxis(i)
  return i == 1 and self.x or i == 2 and self.y or self.z
end
...
function LuaQuat:__tostringDeg()
  return string.format('quat(%.9g,%.9g,%.9g,%.9g)', math.deg(self.x), math.deg(self.y), math.deg(self.z), math.deg(self.w))
end
...
--https://stackoverflow.com/questions/40778955/c-creating-atan2-from-atan
function mathatan2(a, b)
	local res;
	if b > 0 then
		res = math.atan(a / b);
	elseif b < 0 and a >= 0 then
		res = math.atan(a / b) + math.pi;
	elseif b < 0 and a < 0 then
		res = math.atan(a / b) - math.pi;
    elseif b == 0 and a > 0 then
		res = math.pi / 2;
...
function LuaQuat.toEulerYXZ(q)
  ...
  return newLuaVec3xyz(
    mathatan2(2*(q.x*q.y + q.w*q.z), wxsq-yzsq),
    math.asin(max(min(-2*(q.y*q.z - q.w*q.x), 1), -1)),
    mathatan2(2*(q.x*q.z + q.w*q.y), wxsq+yzsq))
end
...
function quatFromDir(dir, up)
  ...
  if abs(dirNorm:dot(k)) > 0.9999 then
    dirNorm = (dirNorm + dirNorm:perpendicularN() * 1e-5):normalized()
  end
