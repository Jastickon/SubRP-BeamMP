...
MP.RegisterEvent("TriggerSubRPstartCargo", "start_work_cargo")
    ...
    for i, route in ipairs(bus_work) do
        if data == route.id then
            M.DataPlayer[user_id].hud.work_status = {
                ...
                pay = route.payment * (tonumber(user.jobs_ratio) or 1),
            }
...
function load_taxi(user_id)
    ...
    for _, lvl_exp in ipairs(taxi_config.lvl_xp) do
        if tonumber(lvl_exp.lvl) == tonumber(taxi_lvl) then
...
function start_job_taxi(user_id, data_json)
    ...
    local user_taxi_lvl
    for _, lvl_exp in ipairs(taxi_config.lvl_xp) do
        if lvl_exp.lvl == tonumber(user.taxi_lvl) then
            user_taxi_lvl = lvl_exp
        end
    end
...
function taxi_trigger_use(user_id, data)
    ...
    if passenger[user_id] == 0 then
        ...
        send_notify(user_id, "Таксист", "Вы забрали пассажира", "success", 7000)
        ...
        passenger[user_id] = ((taxi_config.pay_the_meters * distance) // 10) * user_taxi_lvl.ratio * (tonumber(user.jobs_ratio) or 1)
        ...
        if tonumber(lvl_user.xp) ~= 0 and tonumber(lvl_user.xp) <= tonumber(user.taxi_exp) then

            local update_data = {
                taxi_lvl = tonumber(user.taxi_lvl) +1,
                taxi_exp = 0
            }
            DataManager.update_data("players",  M.DataPlayer[user_id].nick .. ".json", update_data)
...
function open_work_cargo(user_id)
    if M.DataPlayer[user_id].hud.work_status.activi == true and M.DataPlayer[user_id].hud.work_status.name ~= "Грузоперевозки" then
        send_notify(user_id, "Работы", "Вы уже работаете на другой работе", "error", 10000)
        return
    end

    if cargo_user[user_id] and cargo_user[user_id].stage ~= nil and cargo_user[user_id].stage ~= "finished" then
        send_notify(user_id, "Грузоперевозки", "Сначала завершите текущий рейс!", "error", 10000)
        return
    end
    ...
    for _, lvl_exp in ipairs(cargo_config.lvl_xp) do
        if tonumber(lvl_exp.lvl) == tonumber(cargo_lvl) then
            user_cargo_lvl = lvl_exp
        end
    end 

    local routes = {}
    for _, route in ipairs(cargo_config.routes) do
        if route.bonus then
            route.pay = route.pay * 1.5
        end
        route.pay = route.pay * user_cargo_lvl.ratio * (tonumber(user.jobs_ratio) or 1)
...
function start_work_cargo(user_id, data)
    ...
    if cargo_user[user_id] == nil then
        local user = DataManager.load_data("players",  M.DataPlayer[user_id].nick .. ".json") or {}
        local cargo_config = DataManager.load_data("jobs", "cargo.json") or {}
        data_work[user_id] = {work_data = {stop_pos = cargo_config.stop_pos}}
        for _, route in ipairs(cargo_config.routes) do
            if tonumber(route.id) == tonumber(data) then
                if route.bonus then
                    send_notify(user_id, "Грузоперевозки", "Вы выбрали рейс с бонусом!", "success", 7000)
                else
                    send_notify(user_id, "Грузоперевозки", "Вы выбрали рейс!", "success", 7000)
                end

                local ratio = 1
                for _, lvl_exp in ipairs(cargo_config.lvl_xp) do
                    if lvl_exp.lvl == tonumber(user.cargo_lvl) then
                        ratio = lvl_exp.ratio
                        cargo_user[user_id] = cargo_user[user_id] or {}
                        cargo_user[user_id].veh_id = lvl_exp.veh_id
                        cargo_user[user_id].mass = lvl_exp.mass
                        break
                    end
                end

                local pay = (route.bonus and (route.pay * 1.5) or route.pay) * ratio * (tonumber(user.jobs_ratio) or 1)
                ...
                cargo_user[user_id] = {
                    veh_id = cargo_user[user_id].veh_id,
                    veh_name = cargo_user[user_id].veh_name,
                    route = route,
                    start_pos = route.start,
                    stop_pos = route.stop,
                    pay = pay,
                    mass = cargo_user[user_id].mass,
                    ratio = ratio,
                    stage = "loading"
                }
                

                if not M.DataPlayer[user_id].work_vehicle_spawned then
                    delete_veh_all(user_id)
                    spawn_worked_vehicle(user_id, cargo_user[user_id].veh_id)
                    M.DataPlayer[user_id].work_vehicle_spawned = true
                end

                create_triger(user_id, route.start, "cargo " .. user_id)
                break
            end
        end
    end
...
function cargo_trigger_use(user_id, data)
    local cargo = cargo_user[user_id]
    ...
    if cargo.stage == "loading" then
        cargo.stage = "delivery"
        M.DataPlayer[user_id].hud.work_status = {
            activi = true,
            name = "Грузоперевозки",
            pay = cargo.pay
        ...
        send_data_hud(user_id)
        create_triger(user_id, cargo.stop_pos, "cargo " .. user_id)

    elseif cargo.stage == "delivery" then
        if cargo.route.buisnes_id then
            local buisnes_id = cargo.route.buisnes_id
            local buisnes = DataManager.load_data("buisnes", buisnes_id .. ".json") or {}

            local add_mat = cargo_config.mat_add or 100

            for _, lvl_exp in ipairs(cargo_config.lvl_xp) do
                if tonumber(lvl_exp.lvl) == tonumber(user.cargo_lvl) then
                    add_mat = lvl_exp.mass * add_mat
                    break
                end
            end
            if buisnes.mat and buisnes.mat_max then
                buisnes.mat = math.min((buisnes.mat or 0) + add_mat, buisnes.mat_max)
                DataManager.save_data("buisnes", buisnes_id .. ".json", buisnes)
                send_notify(user_id, "Бизнес", "В бизнес '"..(buisnes.name or buisnes_id).."' доставлено материалов: +"..add_mat, "success", 7000)
            end
        end

        if cargo.route.bonus then
            cargo.route.bonus = false
            for _, route in ipairs(cargo_config.routes) do
                if route.id == cargo.route.id then
                    route.bonus = false
                end
            end
            DataManager.save_data("jobs", "cargo.json", cargo_config)
        end

        local new_bal = tostring(tonumber(user.bal) + cargo.pay)
        local update_data = {
            bal = new_bal,
            cargo_exp = (user.cargo_exp or 0) + 1
        }
        DataManager.update_data("players",  M.DataPlayer[user_id].nick .. ".json", update_data)
        M.DataPlayer[user_id].hud.InfoPlayerMoney = new_bal
        send_notify(user_id, "Грузоперевозки", "Вы получили за доставку ".. cargo.pay .."$", "success", 7000)
        send_data_hud(user_id)
        ...
        if user_cargo_lvl and user_cargo_lvl.xp ~= 0 and user_cargo_lvl.xp <= (user.cargo_exp or 0) + 1 then
            local update_lvl = {
                cargo_lvl = user.cargo_lvl + 1,
                cargo_exp = 0
            }
            DataManager.update_data("players",  M.DataPlayer[user_id].nick .. ".json", update_lvl)
            send_notify(user_id, "Грузоперевозки", "Вы получили новый уровень!", "success", 7000)
        end

        cargo_user[user_id] = nil
        M.DataPlayer[user_id].hud.work_status = {
            activi = true,
            name = "Грузоперевозки",
            pay = "Выбор груза"
...
function stop_work(user_id, data)
    cargo_user[user_id] = nil
    M.DataPlayer[user_id].hud.work_status.pay = "Закончите работу"
    send_data_hud(user_id)
    remove_current_pos(user_id)
