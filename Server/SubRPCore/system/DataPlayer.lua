...
function getRender_user_name(user_id)
    ...
    for id, nick in pairs(players) do
        ...
        if is_admin then
            user_list[nick].admin = "Ник: ".. user.nick .. " Рп: ".. user.rp_name .. " SID: " .. id .." ID: ".. user.game_id .. " LVL: ".. user.lvl
        end
    end
    return user_list
end
...
function PlayerJoin(player_id)
    ...
    local user = DataManager.load_data("players", M.DataPlayer[player_id].nick .. ".json")
    if user.last_owner_car then
        spawn_owned_vehicle(player_id, user.last_owner_car)
    end
